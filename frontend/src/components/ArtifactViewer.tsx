...
export const ArtifactViewer: React.FC<ArtifactViewerProps> = ({ artifact, onClose }) => {
  ...
  return (
    <div className={`h-full flex flex-col bg-surface-900 border-l border-surface-800 transition-all duration-300 ${isFullscreen ? 'fixed inset-0 z-50 bg-surface-950' : 'w-full'}`}>
      {/* Top Header Bar */}
      <div className="h-14 px-4 border-b border-surface-800 flex items-center justify-between bg-surface-900/90 backdrop-blur-xs">
        <div className="flex items-center space-x-3 overflow-hidden">
          ...
          <div className="truncate">
            <h3 className="text-sm font-semibold text-surface-100 truncate">{artifact.title}</h3>
        ...
        <div className="flex items-center space-x-1">
          ...
          <div className="flex bg-surface-950 p-0.5 rounded-lg border border-surface-800 mr-2">
            {isHtml && (
              <button
                ...
                className={`px-2.5 py-1 text-xs font-medium rounded-md transition-colors flex items-center space-x-1 ${
                  tab === 'preview' ? 'bg-surface-800 text-surface-50 shadow-xs' : 'text-surface-400 hover:text-surface-200'
                }`}
              >
            ...
            <button
              onClick={() => setTab(isHtml ? 'code' : 'markdown')}
              className={`px-2.5 py-1 text-xs font-medium rounded-md transition-colors flex items-center space-x-1 ${
                (tab === 'code' || tab === 'markdown') ? 'bg-surface-800 text-surface-50 shadow-xs' : 'text-surface-400 hover:text-surface-200'
              }`}
            >
          ...
          {/* Device viewport toggle (only for HTML preview) */}
          {isHtml && tab === 'preview' && (
            <div className="hidden sm:flex bg-surface-950 p-0.5 rounded-lg border border-surface-800 mr-2">
              <button
                onClick={() => setViewport('desktop')}
                title="Desktop View"
                className={`p-1 rounded-md ${viewport === 'desktop' ? 'bg-surface-800 text-lenny-400' : 'text-surface-400 hover:text-surface-200'}`}
              >
              ...
              <button
                ...
                title="Tablet View"
                className={`p-1 rounded-md ${viewport === 'tablet' ? 'bg-surface-800 text-lenny-400' : 'text-surface-400 hover:text-surface-200'}`}
              >
              ...
              <button
                ...
                title="Mobile View"
                className={`p-1 rounded-md ${viewport === 'mobile' ? 'bg-surface-800 text-lenny-400' : 'text-surface-400 hover:text-surface-200'}`}
              >
      ...
      <div className="flex-1 overflow-auto p-4 flex justify-center bg-surface-950/50">
        {isHtml && tab === 'preview' ? (
          <div className={`w-full h-full flex justify-center transition-all duration-200`}>
            <div className={`${getViewportWidth()} h-full shadow-2xl`}>
